...
(function () {
  ...
  /* -------------------------------------------------------
     인증 상태 (localStorage 기반)
  ------------------------------------------------------- */
  function isLoggedIn() {
    try { return localStorage.getItem('lx_logged_in') === '1'; }
    catch (e) { return false; }
  }
  window.AuthState = {
    isLoggedIn: isLoggedIn,
    login: function () { try { localStorage.setItem('lx_logged_in', '1'); } catch (e) {} },
    logout: function () { try { localStorage.removeItem('lx_logged_in'); } catch (e) {} }
  };
  ...
  /* -------------------------------------------------------
     인증 상태에 따라 헤더 메뉴 보이기/숨기기
     - 비로그인: 메가 메뉴(.gnb-nav) + 보조 메뉴(.sub-menu .action-dropdown) 숨김, 로그인 버튼만
     - 로그인: 모두 표시
  ------------------------------------------------------- */
  function applyAuthVisibility() {
    ...
    header.classList.toggle('is-logged-out', !loggedIn);

    var sub = header.querySelector('.sub-menu');
    if (sub && !loggedIn && !sub.querySelector('.gnb-login-btn')) {
      var current = window.location.pathname.split('/').pop() || 'home.html';
      var btn = document.createElement('a');
      btn.className = 'gnb-login-btn';
      btn.href = 'login.html?return=' + encodeURIComponent(current);
      btn.textContent = '로그인';
      sub.appendChild(btn);
    }

    // 비로그인이면 home 의 '서비스 지원' 섹션(.home-board) 자체를 숨김
    var board = document.querySelector('.home-board');
    if (board) board.hidden = !loggedIn;

    // 비로그인이면 visualty1 의 진입 버튼 그룹 숨김 (신규 모델 개발/분석 서비스 바로가기)
    var heroBtns = document.querySelector('.visualty1-center .btn-group');
    if (heroBtns) heroBtns.hidden = !loggedIn;
  }
  window.AuthState.applyVisibility = applyAuthVisibility;

  /* -------------------------------------------------------
     로그아웃 링크 처리 — localStorage 정리 후 home.html 로
  ------------------------------------------------------- */
  function initLogoutLinks() {
    document.querySelectorAll('a[href*="logout"], a[data-action="logout"]').forEach(function (a) {
      var href = a.getAttribute('href') || '';
      if (!/logout/.test(href) && a.dataset.action !== 'logout') return;
      a.addEventListener('click', function (e) {
        e.preventDefault();
        try { localStorage.removeItem('lx_logged_in'); } catch (er) {}
  ...
  var ICONS = {
    ...
    danger:  '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>'
  };
